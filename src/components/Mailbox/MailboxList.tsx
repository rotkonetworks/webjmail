...
export function MailboxList({ onSelect }: Props) {
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="border-b border-bright-black p-2 text-bright-black">MAILBOXES</div>
      {flatMailboxes.map((mailbox, index) => (
        ...
        >
          <span className="flex items-center gap-2">
            <span className="text-bright-black">{mailbox.role === 'inbox' ? '»' : ' '}</span>
            <span>{mailbox.name}</span>
          </span>
          {mailbox.unreadEmails > 0 && <span className="text-cyan">[{mailbox.unreadEmails}]</span>}
        </div>
      ))}
    </div>
  )
